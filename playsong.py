...
def play_songID(s_id, user):
    ...
    if len(lst) == 0: 
        dbaccess.execute_start("INSERT INTO userplayssong VALUES('%s', '%s', '0')")
    dbaccess.execute_start("UPDATE userplayssong SET play_history = '%d' WHERE song_id = %s AND username = '%s'" %  (int(lst[0][3])+1, lst[0][2], user))
